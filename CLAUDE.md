...
The `bins_below` range is a linear function of positive pool volatility (set in the screener prompt, `index.js`), with `totalBins >= 35` (`MIN_SAFE_BINS_BELOW`).
...
| `OPENROUTER_API_KEY` (or `LLM_API_KEY`) | yes | LLM provider key (OpenAI-compatible). |
...
| `HIVE_MIND_URL` / `HIVE_MIND_API_KEY` | no | Override HiveMind defaults. |
...
| `/positions` / `/pool <n>` / `/close <n>` / `/set <n> <note>` | `index.js#telegramHandler` | Bypass LLM — direct state mutation. `/close <n>` calls `closePosition` directly. `/positions` renders a progress bar (`[████████░░░░░░░░░░░░] 40%`, no bin numbers/arrows). |
| `/spectate on\|off` | `index.js#telegramHandler` | Toggle watch-only mode (cycles keep running, no fund/position actions); bare `/spectate` reports state + open-position count. See § Spectate Mode. |
...
- **`lessons.js evolveThresholds()`** evolves `minOrganic` and `minFeeActiveTvlRatio` only. It also references `maxVolatility` + `minFeeTvlRatio` (wrong key names — `minFeeTvlRatio` should be `minFeeActiveTvlRatio`; `maxVolatility` doesn't exist in config at all), so evolution of those keys is a no-op.
- **MiniMax-M2.7 intermittent failures**: the model occasionally fails to make tool calls (returns text-only) or rejects `system`-role messages. Model-level, not a code bug — manifests as "I couldn't complete that reliably because no tool call was made" in cycles. No code fix needed; retries on the next cron cycle.
