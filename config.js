...
if (u.llmBaseUrl) {
  process.env.LLM_BASE_URL ||= u.llmBaseUrl;
  process.env.OPENAI_BASE_URL ||= u.llmBaseUrl;
}
if (u.llmApiKey) {
  process.env.LLM_API_KEY ||= u.llmApiKey;
  process.env.OPENAI_API_KEY ||= u.llmApiKey;
  process.env.MINIMAX_API_KEY ||= u.llmApiKey;
}
...
export const config = {
  ...
  gmgn: {
    ...
    // "gmgn" = use GMGN OpenAPI for the minTokenFeesSol fee gate; "jupiter" = fall back to Jupiter's fee figure.
  ...
  llm: {
    ...
    managementModel: u.managementModel ?? DEFAULT_MINIMAX_MODEL,
    screeningModel:  u.screeningModel  ?? DEFAULT_MINIMAX_MODEL,
    generalModel:    u.generalModel    ?? DEFAULT_MINIMAX_MODEL,
  },
