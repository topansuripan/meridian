...
const g  = (key, fallback) => existingGmgn[key] ?? fallback;
...
const screeningSourceChoice = await askChoice("Screening source:", [
  { label: "meteora — legacy Meteora pool discovery",                        key: "meteora" },
  { label: "gmgn    — GMGN token scan then Meteora DLMM pool match",         key: "gmgn"    },
], { defaultKey: e("screeningSource", "meteora") });
const screeningSource = screeningSourceChoice.key;
...
const maxBotHoldersPct = await askNum(
  "Max bot holders % (0–100)",
  p("maxBotHoldersPct", e("maxBotHoldersPct", 30)),
  { min: 0, max: 100 }
);

const maxTop10Pct = await askNum(
  "Max top-10 holder concentration % (0–100)",
  p("maxTop10Pct", e("maxTop10Pct", 60)),
  { min: 0, max: 100 }
);

const athFilterPctRaw = await ask(
  "ATH filter % — reject tokens within this % of all-time high (blank/null = disabled)",
  e("athFilterPct", "null")
);
const athFilterPct =
  athFilterPctRaw === undefined || String(athFilterPctRaw).trim() === "" || String(athFilterPctRaw).trim().toLowerCase() === "null"
    ? null
    : parseFloat(athFilterPctRaw);

// ─── Section 5b: GMGN Screening (used when screeningSource = gmgn) ─────────────
console.log("\n── GMGN Screening ────────────────────────────────────────────");
console.log(screeningSource === "gmgn"
  ? "GMGN is your active screening source — configure it below."
  : "GMGN is not the active source, but you can still configure it (used for the fee gate).");

const gmgnApiKeyRaw = await ask(
  "GMGN API key",
  existingGmgn.apiKey ? "*** (already set)" : (existingEnv.GMGN_API_KEY || "")
);
const gmgnApiKey = String(gmgnApiKeyRaw).startsWith("***")
  ? (existingGmgn.apiKey || "")
  : gmgnApiKeyRaw;

const gmgnInterval = (await askChoice("GMGN trending interval:", [
  { label: "1m",  key: "1m"  },
  { label: "5m",  key: "5m"  },
  { label: "1h",  key: "1h"  },
  { label: "6h",  key: "6h"  },
  { label: "24h", key: "24h" },
], { defaultKey: g("interval", "5m") })).key;

const gmgnLimit = await askNum(
  "GMGN rank limit",
  g("limit", 100),
  { min: 1 }
);

const gmgnEnrichLimit = await askNum(
  "GMGN enrich shortlist size",
  g("enrichLimit", 20),
  { min: 1 }
);

const gmgnMinMcap = await askNum(
  "GMGN min market cap USD",
  g("minMcap", 150_000),
  { min: 0 }
);

const gmgnMaxMcap = await askNum(
  "GMGN max market cap USD",
  g("maxMcap", 10_000_000),
  { min: 0 }
);

const gmgnMinVolume = await askNum(
  "GMGN min 5m volume USD",
  g("minVolume", 1_000),
  { min: 0 }
);

const gmgnMinHolders = await askNum(
  "GMGN min holders",
  g("minHolders", 1_000),
  { min: 0 }
);

const gmgnMinTotalFeeSol = await askNum(
  "GMGN min total fee SOL",
  g("minTotalFeeSol", 30),
  { min: 0 }
);

const gmgnRequireKol = await askBool(
  "Require active KOL holder?",
  g("requireKol", true)
);
...
// ─── Section 6d: Pump Entry Guard (NORMAL only) ──────────────────────────────
console.log("\n── Pump Entry Guard ──────────────────────────────────────────");

const maxPump5mPct = await askNum(
  "Max single 5m candle pump % before entry is blocked",
  e("maxPump5mPct", 20),
  { min: 0 }
);

const maxPump15mPct = await askNum(
  "Max 15m (3-candle) pump % before entry is blocked",
  e("maxPump15mPct", 30),
  { min: 0 }
);

const pumpLookbackHours = await askNum(
  "Pump lookback window (hours)",
  e("pumpLookbackHours", 2),
  { min: 0 }
);

// ─── Section 6e: Fast-Drop Recovery-Exit Guard (NORMAL only) ─────────────────
console.log("\n── Fast-Drop Guard ───────────────────────────────────────────");

const fastDropGuardEnabled = await askBool(
  "Enable fast-drop recovery-exit guard?",
  e("fastDropGuardEnabled", true)
);

const fastDropWindowMinutes = await askNum(
  "Fast-drop rolling window (minutes)",
  e("fastDropWindowMinutes", 15),
  { min: 1 }
);

const fastDropTriggerPct = await askNum(
  "Fast-drop trigger % (arm recovery watch on drop from rolling high)",
  e("fastDropTriggerPct", 15),
  { min: 0 }
);

const fastDropBouncePct = await askNum(
  "Fast-drop bounce % off the low to exit",
  e("fastDropBouncePct", 10),
  { min: 0 }
);

const fastDropHardFloorPct = await askNum(
  "Fast-drop hard floor % (force exit at this PnL)",
  e("fastDropHardFloorPct", -25)
);
...
const providerChoice = await askChoice("Select LLM provider:", LLM_PROVIDERS.map((prov) => ({ label: prov.label, key: prov.key })));
const provider = LLM_PROVIDERS.find((prov) => prov.key === providerChoice.key);
...
const llmModel = await ask(
  "Default model name",
  e("llmModel", process.env.LLM_MODEL || provider.modelDefault || DEFAULT_MODEL)
);
...
const spectateMode = await askBool(
  "Spectate mode? (watch-only — cycles run but no fund/position actions)",
  e("spectateMode", false)
);
...
// ─── Write gmgn-config.json ─────────────────────────────────────────────────────
const gmgnConfig = {
  ...existingGmgn,
  interval: gmgnInterval,
  limit: gmgnLimit,
  enrichLimit: gmgnEnrichLimit,
  minMcap: gmgnMinMcap,
  maxMcap: gmgnMaxMcap,
  minVolume: gmgnMinVolume,
  minHolders: gmgnMinHolders,
  minTotalFeeSol: gmgnMinTotalFeeSol,
  requireKol: gmgnRequireKol,
  ...(gmgnApiKey ? { apiKey: gmgnApiKey } : {}),
};
if (!gmgnConfig.apiKey && existingGmgn.apiKey) gmgnConfig.apiKey = existingGmgn.apiKey;
...
const userConfig = {
  ...
  maxPump15mPct,
  pumpLookbackHours,
  // Fast-drop recovery-exit guard
  fastDropGuardEnabled,
  fastDropWindowMinutes,
  fastDropTriggerPct,
  fastDropBouncePct,
  fastDropHardFloorPct,
  // Exit rules
  takeProfitPct,
  stopLossPct,
...
╚═══════════════════════════════════════════════╝
  ...
  Spectate:     ${spectateMode ? "YES — watch-only" : "NO"}
  ...
  Source:       ${screeningSource}${screeningSource === "gmgn" ? (gmgnConfig.apiKey ? " (GMGN key configured)" : " (GMGN key not set)") : ""}
  ...
  Holders:      bot ≤ ${maxBotHoldersPct}%  ·  top10 ≤ ${maxTop10Pct}%  ·  ATH filter ${athFilterPct === null ? "disabled" : `${athFilterPct}%`}
  ...
  Pump guard:   5m ≤ ${maxPump5mPct}%  ·  15m ≤ ${maxPump15mPct}%  ·  lookback ${pumpLookbackHours}h
  Fast-drop:    ${fastDropGuardEnabled ? `trigger ${fastDropTriggerPct}%, bounce ${fastDropBouncePct}%, floor ${fastDropHardFloorPct}% (${fastDropWindowMinutes}m window)` : "disabled"}
